...
datos_2015 <- datos_2015 %>%
  mutate(Partido = case_when(
    Partido == "NA" ~ "NUAL",
    str_detect(Partido, "_NA$") ~ str_replace(Partido, "_NA$", "_NUAL"),
    str_detect(Partido, "^NA_") ~ str_replace(Partido, "^NA_", "NUAL_"),
    str_detect(Partido, "_NA_") ~ str_replace(Partido, "_NA_", "_NUAL_"),
    TRUE ~ Partido
  ))
```
...
```{r}
datos_2018 <- calcular_votos_coalicion(datos_2018)
```
...
```{r}
datos_2019 <- calcular_votos_coalicion(datos_2019)
```
...
```{r}
datos_2021 <- calcular_votos_coalicion(datos_2021)
```
...
```{r}
datos_2022 <- calcular_votos_coalicion(datos_2022)
```
